...
def get_label(file_path: str) -> str:
    return 'voice' if 'voice' in file_path else 'not_voice'
...
def get_csv(csv_name: str, data_split: list, annotations_path: str) -> None:
    with open(csv_name, mode='w', newline='') as f:
        ...
        for filename in data_split:
            try:
                print(csv_name + ' THREAD: Applying transform to: ' + filename)
    ...
    shutil.move(csv_name, os.path.join(annotations_path, csv_name))


def prepare_ffnn_dataset(annotation_path: str, src_path: str) -> None:
    label_1 = 'voice'
    label_0 = 'not_voice'

    label_1_path = os.path.join(src_path, label_1)
    label_0_path = os.path.join(src_path, label_0)

    label_1_wavs = [os.path.join(label_1_path, file) for file in os.listdir(label_1_path) if file.endswith(".wav")]
    label_0_wavs = [os.path.join(label_0_path, file) for file in os.listdir(label_0_path) if file.endswith(".wav")]
    ...
    train = label_1_wavs[:label_1_size] + label_0_wavs[:label_0_size]
    test = label_1_wavs[label_1_size:] + label_0_wavs[label_0_size:]

    get_csv('train.csv', train, annotation_path)

    get_csv('test.csv', test, annotation_path)
